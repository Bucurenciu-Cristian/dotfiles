...
def is_critical_typescript_error(error_text):
    """Check if a TypeScript error is critical (blocks compilation)."""
    critical_patterns = [
        "Cannot find module",
        "Module not found",
    ...
    # Skip Next.js generated type errors that are often false positives
    if ".next/types/" in error_text:
        return False
...
def parse_lint_output(output):
    """Parse lint output to separate critical errors from warnings."""
    lines = output.split('\n')
    critical_errors = []
    warnings = []
    ...
    for line in lines:
        ...
        if " Error: " in line:
            ...
            if any(pattern in line for pattern in [
                ...
                # Most ESLint "errors" are actually warnings in terms of blocking commits
                warnings.append(line)
        elif " Warning: " in line:
            warnings.append(line)
        elif "error" in line.lower() and ("parse" in line.lower() or "syntax" in line.lower()):
            critical_errors.append(line)
...
def main():
    try:
        ...
        # Check for hook strictness level
        strict_mode = os.getenv('CLAUDE_HOOK_STRICT', 'false').lower() == 'true'
        ...
        if project_info['has_typescript'] and project_info['ts_check_command']:
            ...
            if not success:
                ...
                if strict_mode:
                    critical_errors.append(f"TypeScriptl errors found:\n{error_output}")
                else:
                    # Check for critical TypeScript errors
                    if is_critical_typescript_error(error_output):
                        critical_errors.append(f"Critical TypeScript errors found:\n{error_output}")
                    else:
                        warnings.append(f"TypeScript warnings found:\n{error_output}")
        ...
        if project_info['has_lint'] and project_info['lint_command']:
            ...
            if not success:
                lint_output = stderr or stdout
                if strict_mode:
                    critical_errors.append(f"Linting errors found:\n{lint_output}")
                else:
                    # Parse lint output to separate critical errors from warnings
                    crit_errors, lint_warnings = parse_lint_output(lint_output)
                    if crit_errors:
                        critical_errors.append(f"Critical linting errors found:\n" + "\n".join(crit_errors))
                    if lint_warnings:
                        warnings.append(f"Linting warnings found:\n" + "\n".join(lint_warnings))

        # Run tests if available (only for commits, not pushes to save time)
        # Tests are always critical - if they fail, block the commit
        if 'git commit' in command and project_info['has_tests'] and project_info['test_command']:
            ...
            if not success:
                critical_errors.append(f"Test failures found:\n{stderr or stdout}")

        # If there are critical errors, block the commit
        if critical_errors:
            print("Pre-commit checks failed!\n", file=sys.stderr)
            for error in critical_errors:
                print(f"❌ {error}\n", file=sys.stderr)
            print("Fix these critical issues before committing.", file=sys.stderr)
            sys.exit(2)  # Exit code 2 blocks the tool call

        # Show warnings but allow commit
        if warnings:
            print("Pre-commit checks completed with warnings:\n", file=sys.stderr)
            for warning in warnings:
                print(f"⚠️  {warning}\n", file=sys.stderr)
            print("Warnings found but commit is allowed. Consider fixing these issues.", file=sys.stderr)
